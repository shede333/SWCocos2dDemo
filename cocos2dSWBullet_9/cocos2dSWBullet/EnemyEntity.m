...
- (void)goHit{
    ...
    if (hitNum <= 0) {
        ...
        CCParticleSystem *sys = [ARCH_OPTIMAL_PARTICLE_SYSTEM particleWithFile:@"fx-explosion.plist"];
        sys.positionType = kPositionTypeFree;
        sys.autoRemoveOnFinish = YES;
        sys.position = self.position;
        [[GameScene shareGameScene] addChild:sys];
    }
